...
import { sendClosingNotifications } from '@/lib/push';
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Recordatorios "cierra en ~2 h" (no es fatal si falla).
    let notified = 0;
    try {
      notified = await sendClosingNotifications();
    } catch {}
    return NextResponse.json({ ok: true, ...summary, notified });
  } catch (e) {
