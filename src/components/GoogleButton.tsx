...
export function GoogleButton({ label = 'Continuar con Google' }: { label?: string }) {
  ...
  async function signIn() {
    ...
    if (error) setError('Google no está disponible en este momento.');
  }
  ...
  return (
    <div>
      <button
        type="button"
        onClick={signIn}
        className="flex w-full items-center justify-center gap-2.5 rounded-lg border border-slate-300 bg-white px-4 py-2.5 font-semibold text-slate-700 transition hover:bg-slate-50"
      >
